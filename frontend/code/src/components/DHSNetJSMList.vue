            ...
            <template v-slot:prepend>
              <q-icon name="event" class="cursor-pointer">
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                  <q-date v-model="localJSMSet.sTime" mask="YYYY-MM-DD HH:mm:ss">
        ...
        <div class="col q-pa-sm bg-grey-2">
          ...
          <q-input class="" filled v-model="localJSMSet.eTime">
            <template v-slot:prepend>
              <q-icon name="event" class="cursor-pointer">
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                  <q-date v-model="localJSMSet.eTime" mask="YYYY-MM-DD HH:mm:ss">
                    <div class="row items-center justify-end">
                      <q-btn v-close-popup label="Close" color="primary" flat />
                    </div>
                  </q-date>
          ...
          <q-tab-panels v-model="tabofCard" animated>
            ...
            <q-tab-panel name="two">
              ...
              <section @click="adjustJSMCategory">
                <div
                  class="row q-py-sm q-px-sm bg-teal-1 cursor-pointer"
                  v-if="all.custom_category !== ''"
                >
                  <span class="col">Category NET</span>
              ...
              <section @click="adjustJSMinfra">
                <div
                  class="row q-py-sm q-px-sm bg-cyan-1 cursor-pointer"
                  v-if="all.custom_infra.length !== 0"
                >
              ...
              <section @click="adjustJSMFacilities">
                <div
                  class="row q-py-sm q-px-sm bg-teal-1 cursor-pointer"
                  v-if="all.custom_facilities.length !== 0"
                >
                  <span class="col">Facilities</span>
                  <span class="col">{{ all.custom_facilities.join(', ') }}</span>
                </div>
                <div
                  class="row q-py-sm q-px-sm bg-teal-1 cursor-pointer text-red-6 text-italic"
                  v-else
              ...
              <section @click="adjustJSMStartTime">
                <div
                  v-if="all.startTime !== '1989-11-09 08:00:00' && all.startTime"
                  class="row q-py-sm q-px-sm bg-cyan-1 cursor-pointer"
                >
              ...
              >
                ...
                <span
                  v-if="all.endTime !== '1989-11-09 08:00:00' && all.endTime"
                  class="col"
                  >{{ all.endTime }}</span
                >
                <span v-else class="col">None</span>
              ...
              <div
                @click="adjustJSMVendor"
                class="row q-py-sm q-px-sm bg-cyan-1 cursor-pointer"
              >
                <span class="col">Vendor Support</span>
                <span v-if="all.custom_vendor" class="col">{{
                  all.custom_vendor.join(', ')
                }}</span>
                <span v-else class="col">None</span>
              </div>
              <div
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const localJSMSet = reactive({
      ...
      facilities: props.custom_facilities,
      infra: props.custom_infra,
      vendors: props.custom_vendor ? props.custom_vendor : [],
    ...
    async function adjustJSMStatus() {
      ...
      try {
        ...
        $q.dialog({
          ...
          axios
            .post(
              `https://${ServiceDomainLocal}:9487/bpCowork/jsm/net/update/status`,
              postData
            )
            .then((res) => {
              $q.notify({
    ...
    async function adjustJSMCategory() {
      ...
      const res = await axios.get(
        `https://${ServiceDomainLocal}:9487/bpCowork/jsm/query/field/customfield_10286/1`
      )
      // popup windows
      $q.dialog({
        ...
        options: {
          ...
          items: res.data,
          isValid: (val) => val !== localJSMSet.category
        },
        cancel: true,
        persistent: true
      }).onOk((data) => {
    ...
    async function adjustJSMFacilities() {
      // get option list by backend
      const res = await axios.get(
        `https://${ServiceDomainLocal}:9487/bpCowork/jsm/query/field/customfield_10287/1`
      )
      // popup windows
      $q.dialog({
        title: `Change ${props.issueKey} facilities`,
        options: {
          type: 'checkbox',
          model: localJSMSet.facilities,
          items: res.data,
          isValid: (val) => val.length !== 0
        },
        cancel: true,
        ...
        updateToJSM(postData, 'facilities')
        // need this one to update local data
        localJSMSet.facilities = data
      })
    }
    ...
    async function adjustJSMinfra() {
      // get option list by backend
      const res = await axios.get(
        `https://${ServiceDomainLocal}:9487/bpCowork/jsm/query/field/customfield_10264/1`
      )
      // popup windows
      $q.dialog({
    ...
    async function adjustJSMVendor() {
      // get option list by backend
      const res = await axios.get(
        `https://${ServiceDomainLocal}:9487/bpCowork/jsm/query/field/customfield_10288/1`
      )
      // popup windows
      $q.dialog({
        title: `Change ${props.issueKey} vendor support`,
        options: {
          type: 'checkbox',
          model: localJSMSet.vendors,
          items: res.data
        },
        cancel: true,
        persistent: true
        ...
        updateToJSM(postData, 'vendor')
        // need this one to update local data
        localJSMSet.vendors = data
      })
    }
    ...
    async function updateToJSM(payload, target) {
      ...
      await axios
        .post(
          `https://${ServiceDomainLocal}:9487/bpCowork/jsm/net/update/option`,
          payload
        )
        .then((res) => {
          if (res.status === 200) {
    ...
    async function pullJSMTicket() {
      ...
      barRef.start()
      await axios
        .post(`https://${ServiceDomainLocal}:9487/bpCowork/jsm/pull/net`, postData)
        .then((res) => {
          if (res.status === 200) {
            $q.notify({
    ...
    watch(
      ...
      (curValue, oldValue) => {
        console.log('hit watch on NET LIST WATCH')
