...
<template>
  <div v-if="content_html" class="row bg-cyan-1 text-bold">
    <span class="col self-center q-pl-sm"> Email Content </span>
    <span class="col text-right">
      <q-toggle
        v-model="isShowContentHtmlMtn"
        checked-icon="check"
        color="green"
  ...
  <div class="row q-pa-sm" v-if="isShowContentHtmlMtn">
  ...
  <q-stepper v-model="mtnObject.mtnStep" ref="stepper" color="primary" animated>
    ...
    >
      ...
      <q-card v-if="mtnGroup === 'other'" flat bordered>
        ...
        <q-card-section horizontal>
          <q-card-section class="col">
            <q-input
              standout="bg-yellow-9 text-bold text-dark"
    ...
    >
      ...
      <div class="row">
        <span class="q-ml-md">{{ ticketJiraIssueKey }} - {{ ticketSubject }}</span>
      </div>
      <div class="row">
...
export default defineComponent({
  ...
  props: {
    ...
    ticketNumber: Number,
    ticketJiraIssueKey: String,
    ticketJiraIssueId: String,
    ticketMtn: Boolean,
    editor: String,
    content_html: String
  },
  emits: ['updateSummaryByMtn', 'updateParentComment', 'adjustQTabStatus'],
  setup(props, context) {
    const $q = useQuasar()
    const $store = useStore()
    const ServiceDomainLocal = $store.state.fixedConfig.serviceDomain
    const mtnGroup = ref('')
    const isShowContentHtmlMtn = ref(false)
    const mtnOptions = reactive([
      ...
      { label: 'Others', value: 'other', color: 'yellow-9' }
    ])
    ...
    function toTriggerComment() {
      console.log('update comment, post the emit - to JSMList')
      context.emit('updateParentComment')
    }

    // google calendar create new event part
    ...
    function createNewEvent() {
      ...
      const event = {
        summary: `${props.ticketJiraIssueKey} - ${props.ticketSubject}`,
          ...
          function (response) {
            ...
            if (response.status === 200) {
              ...
              const postData = reactive({
                ...
                _source: 'jira',
                _sn: props.ticketNumber,
                _editor: props.editor,
                _issueKey: props.ticketJiraIssueKey
              })
              axios
    ...
    watch(
      ...
      (curStep, prevStep) => {
        switch (curStep) {
          case 1:
            console.log('step 1')
            context.emit('adjustQTabStatus', 0)
            break
          case 2:
            console.log('step 2')
            context.emit('adjustQTabStatus', 1)
            break
          case 3:
            console.log('step 3')
            context.emit('adjustQTabStatus', 1)
