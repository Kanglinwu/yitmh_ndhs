  ...
  <section v-else>
    ...
    <div v-if="kpiResult.type === '3'" class="row">
      ...
      <q-uploader
        ...
        class="col-6"
        :headers="[
          { name: 'dbSn', value: targetNoteSn },
          { name: 'zone', value: 'note' }
        ]"
...
export default defineComponent({
  name: 'DHSNoteListKpi',
  emits: ['adjustNoteKpiOnly'],
  props: {
    targetKpiSn: Number,
    targetKpiGroup: Number,
    targetNoteSn: Number
  },
  setup(props, context) {
    const $q = useQuasar()
    ...
    function resetButton() {
      ...
      if (props.targetKpiSn) {
        const postData = reactive({
          ...
          originSourceSn: props.targetNoteSn,
          _type: 'reset'
        })
        axios
          ...
          .then((res) => {
            ...
            context.emit('adjustNoteKpiOnly', res.data)
          })
          .catch((error) => {
            console.log(error)
          })
      ...
      if (type === '1') {
        ...
        axios
          ...
          .then((res) => {
            ...
            kpiResult.related_sn = props.targetNoteSn
            kpiResult.related_group = res.data.kpiGroup
    ...
    onMounted(async () => {
      ...
      if (props.targetKpiSn !== 0) {
        const res = await axios.get(
          `https://${ServiceDomainLocal}:9487/notes/kpi/query/${props.targetKpiSn}`
        )
        console.log(`Note KPI record - type is ${res.data.type}, sn is ${props.targetNoteSn}`)
        kpiResult.sn = res.data.sn
        kpiResult.type = res.data.type
        kpiResult.resolve_status = res.data.resolve_status
        kpiResult.handler = res.data.handler
