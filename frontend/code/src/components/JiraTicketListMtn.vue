...
<template>
  <q-stepper v-model="mtnObject.mtnStep" ref="stepper" color="primary" animated>
    ...
    >
      ...
      <div class="row">
        <span class="q-ml-md">[{{ ticketNumber }}]-{{ ticketSubject }}</span>
      </div>
      <div class="row">
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const mtnOptions = reactive([
      ...
      { label: 'Others', value: 'other', color: 'yellow', disable: true }
    ])
    ...
    function createNewEvent() {
      ...
      const event = {
        summary: `[${props.ticketNumber}]-${props.ticketSubject}`,
          ...
          axios
            ...
            .then((res) => {
              if (Object.keys(res.data).length) {
                ...
                console.log(
                  `${props.ticketNumber} mtn button has been enabled, but user did not update the google calendar api accordingly`
                )
              }
            })
        } catch (error) {
