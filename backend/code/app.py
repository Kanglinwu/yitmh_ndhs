...
def default_index(target=None):
    ...
    # system check the ticket, if already exist DB just ignore elsewise will add it.
        ...
        if len(checkDbStatus) == 0:
            ...
            return redirect(
                "https://okta.opsware.xyz:9487/query/all_issues/handover",
                code=302)
        ...
        cLimit = 20
        cStart = 0
        url = f"https://anyhow-test4u.atlassian.net/rest/servicedeskapi/servicedesk/1/queue/16/issue?limit={cLimit}&start={cStart}"
            ...
            for item in target['values']:
                ...
                if int(item['id']) not in ticket_jira_key_id_mapping:
                    insertMappingDb = get_handover_jira_key_id_mapping(
                        ...
                        _id=item['id'],
                        createdAt=curTime,
                        url='https://anyhow-test4u.atlassian.net/jira/servicedesk/projects/YTS/queues/custom/16/{}'
                        .format(item['key']))
...
def mtnIsert():
    ...
    if source == 'jira':
        try:
            dbSn = front_data['_sn']
            newEditor = front_data['_editor']
            targetJsmIssueKey = front_data['_issueKey']
        except Exception as e:
            print(e)
            print('Get issue during parse frontend postData')

        try:
            comment = 'add MTN information'
            ...
            queryBySn = get_jsm_ops_prod.query.filter(
                get_jsm_ops_prod.sn == dbSn).first()
            if queryBySn.comments:
                tmpList = json.loads(queryBySn.comments)
                tmpList.append(commentDbSn)
                queryBySn.comments = json.dumps(tmpList)
            else:
                queryBySn.comments = json.dumps([commentDbSn])
            queryBySn.mtn = 1
            db.session.commit()
            db.session.close()
            db.session.remove()
            ...
            print(e)
            print('Get issue during udpate the local db')
...
def mtnDelete():
    ...
    if resource == 'jira':
        ...
        targetJsmIssueKey = front_data['_issueKey']
        newEditor = front_data['_editor']
        comment = 'delete MTN information'
        # update the comment history -
        addOneRow = get_jsm_ops_comments(issueKey=targetJsmIssueKey,
                                         handler=newEditor,
                                         content=comment)
...
def notesTraceLog(source, targetTitle):
    ...
    elif source == 'fromIncident':
        queryNoteDbByTitle = get_handover_notes.query.filter(
            get_handover_notes.sn == int(targetTitle)).all()
    else:
        queryNoteDbByTitle = get_handover_notes.query.filter(
...
@app.route('/icp/query')
def icpQuery():
    result = requests.get('http://10.7.6.205:5066/axios_querying_icp_db/')
...
def icpCheck():
    try:
        result = requests.get('http://10.7.6.205:5066/selenium_check_icp')
        if result.text == '1':
            return '200'
        else:
            return '500'
            ...
            if value == 'Green':
                ...
                try:
                    for key1, value1 in enumerate(queryCustomerStatusNote):
                        ...
                        print(f'scrollTargetList = {scrollTargetList}')
...
def customerStatusDeletePost():
    ...
    print(len(adjustCustomerList))
    if len(adjustCustomerList) == 0:
        db.session.delete(selectBySn)
        db.session.commit()
        db.session.close()
    else:
...
@app.route('/skypebot')
def skypebotupdate():
    curSkypeSession = Skype("kisl@hotmail.com.tw", "gP_Pqs?Y/sQ^")
    # print(curSkypeSession.user)
    # print(curSkypeSession.contacts)
    # print(curSkypeSession.chats)
    curSkypeSession.setMood('Test by Bot')
    return 'ok'
...
def shiftTableRouting(curData=None, curShift=None):
    if request.method == 'GET':
        try:
            ...
            queryResult = get_handover_shift_table.query.order_by(get_handover_shift_table.date == time_obj, get_handover_shift_table.shift == curShift ).first()
        ...
        while skypeCheck:
            try:
                curSkypeSession = Skype("yt.ops", "QQe*a@a7YE")
...
def shiftleaderapiqueryraw():
    ...
    if isinstance(targetDate, str):
        ...
        result = requests.get(f'http://10.7.6.185:777/ops/{time_str_with_format}/{targetShift}/')
        try:                
            resultJson = result.json()
            print(resultJson['members'])
            returnDict = {}
            # check the local db
            ...
            returnDict['teammates'] = resultJson['members']
